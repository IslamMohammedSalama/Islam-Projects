...
def create_bricks(level):
    ...
    brick_cols = 9
    bricks = []
    special_brick = None  # Reset special brick
    for i in range(brick_rows):
        for j in range(brick_cols):
            if random.choice([True, False]):
                ...
                bricks.append(brick)
    if bricks:
        special_brick = random.choice(bricks)  # Choose a random brick as the special brick
    return bricks
...
def main():
    ...
    while running:
        ...
        for brick in bricks:
            if brick == special_brick:
                pygame.draw.rect(screen, GREEN, brick)  # Draw special brick in green
            else:
